...
pub struct WsStream {
    ...
    //
    closer: Rc<RefCell<Option<Pin<Box<dyn Future<Output = ()>>>>>>,
}
...
impl WsStream {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            sink_waker,
            pharos,
            closer: Rc::new(RefCell::new(None)),
        ...
        match state {
            ...
            _ => {
                ...
                if self.closer.borrow().is_none() {
                    ...
                    self.closer = Rc::new(RefCell::new(Some(closer.boxed())));
                }

                let _ = ready!(Pin::new(&mut self.closer.borrow_mut().as_mut().as_mut().unwrap()).poll(cx));
