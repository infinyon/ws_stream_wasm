...
//
use {
    futures::io::*, futures::prelude::*, futures::sink::*, log::*, pharos::*,
    wasm_bindgen::prelude::*, wasm_bindgen_futures::spawn_local, wasm_bindgen_test::*,
    ws_stream_wasm::*,
};
...
async fn round_trip_text() {
    ...
    wsio.send(WsMessage::Text(message.clone()))
        .await
...
async fn round_trip_binary() {
    ...
    wsio.send(WsMessage::Binary(message.clone()))
        .await
        .expect_throw("Failed to write to websocket");
...
async fn send_while_closing() {
    ...
    let res = wsio
        .send(WsMessage::Text("Hello from browser".into()))
        .await;
...
async fn close_event_from_async_write() {
    ...
    AsyncWriteExt::close(&mut stream)
        .await
        .expect_throw("close ws");
...
async fn debug() {
    ...
    assert_eq!(
        format!("WsStream for connection: {}", URL),
        format!("{:?}", wsio)
    );
